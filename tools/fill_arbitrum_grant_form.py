...
def fill_arbitrum(headless=False):
    ...
    with sync_playwright() as p:
        ...
        comments_text = (
            ...
            "with kernel-only 1.6-3.9 ms (median of 3 runs; end-to-end cold including ~200 ms JIT/init) on consumer hardware "
            "(AMD Radeon RX 6600). Kernel-only throughput is not comparable to L1 TPS (no consensus/network/storage).\n\n"
            ...
            "64-byte SHA-256 Merkle receipt verified on-chain in 70,133 gas (settleBatch, Foundry v1.8.1 on Anvil; "
            "87,831 gas with inclusion proof), we reduce L1 batch posting calldata costs by over 90% during high-volatility events.\n"
            ...
            "and u256 AMM math engine are directly portable to WebAssembly (WASM) as an Arbitrum Stylus native contract (port not yet built; "
            "gas savings unmeasured — target to be benchmarked in Milestone 1).\n\n"
            "3. Repositories & Reproducibility:\n"
            "• Public GitHub: https://github.com/kbelludoo/lin-open (MIT / Apache-2.0)\n"
            "• Live EVM Tested Contract: contracts/LinReceiptVerifier.sol (70,133 gas settleBatch / 87,831 with inclusion proof on EVM)\n"
