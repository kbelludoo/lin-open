// LIN Adversarial Closed-Loop Debugging Agent
...
import { DynamicReRanker } from '../runtime/dynamic_reranker.mjs';
...
export class AdversarialAgent {
  constructor({
    ...
    this.reranker = new DynamicReRanker(this.substrate, this.ledger);
  }
  ...
  async runTask({ task, repoDir, arm = 'LIN_DYNAMIC', maxAttempts = 3 }) {
    this.ledger.clear();
    let historyBuffer = [];
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (arm === 'RAW') {
        ...
        const rankings = this.reranker.computeDynamicRankings(entrySym, task.test_assertion_error, []);
        activeCandidate = rankings[0];
      ...
      } else if (arm === 'LIN_DYNAMIC') {
        // True Dynamic Re-Ranking: computes score at t conditioned on refutations
        const dynamicRankings = this.reranker.computeDynamicRankings(entrySym, task.test_assertion_error, refutedSymbols);
        activeCandidate = dynamicRankings[0] || { symbol: entrySym, module: 'src/ranges.js', score: 0.5 };

        activeHyp = this.ledger.registerHypothesis({
          symbol: activeCandidate.symbol,
          module: activeCandidate.module,
          reason: `Dynamic score ${activeCandidate.score} with live neighbor propagation`
        });

        const focused = this.substrate.generateFocusedContext(activeCandidate.symbol);
        const searchMemory = this.ledger.generateSearchMemoryPrompt();

        prompt = `[LIN Dynamic Re-Ranker State]\n`;
        prompt += `* Target Candidate: ${activeCandidate.symbol} (${activeCandidate.module}) | Dynamic Score: ${activeCandidate.score}\n`;
        prompt += `* Active Invariant Density: ${activeCandidate.invariantsCount || 0} contracts attached\n`;
      ...
      if (testRes.passed) {
        ...
        if (activeCandidate) this.reranker.recordAttemptOutcome(activeCandidate.symbol, true);
        break;
      } else {
        resetFiles();
        historyBuffer.push({ attempt, file: targetFile, testOutput: testRes.output.slice(0, 200) });
        if (activeCandidate) {
          refutedSymbols.push(activeCandidate.symbol);
          this.reranker.recordAttemptOutcome(activeCandidate.symbol, false);
        }
        if (activeHyp) {
