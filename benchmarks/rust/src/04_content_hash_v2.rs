...
// Pre-compile regex patterns (LazyLock = compile once, reuse forever)
static WS_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\s+").unwrap());
static SEMI_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r";\s*").unwrap());
...
fn canonicalize(_fn_name: &str, params: &str, body: &str) -> String {
    ...
    // Collapse whitespace (pre-compiled regex)
    canon = WS_RE.replace_all(&canon, " ").to_string();
    
    // Parse params
    let param_list: Vec<&str> = params.split(',').collect();
    let mut clean: Vec<String> = Vec::with_capacity(param_list.len());
    for p in param_list {
        let trimmed = p.trim();
    ...
    // Build regex on the fly but reuse the pattern
    ...
    // Collapse semicolons (pre-compiled regex)
...
fn build_content_registry(fns: &[Fn]) -> HashMap<String, HashMap<String, String>> {
    let mut registry: HashMap<String, HashMap<String, String>> = HashMap::with_capacity(fns.len());
    for f in fns {
