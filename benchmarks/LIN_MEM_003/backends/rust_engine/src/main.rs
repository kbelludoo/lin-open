...
// --- 3. LIN SEMANTIC ENGINE V3 (CANONICAL COMPOSITE INDEX OVER RESIDENT DAG) ---
...
impl LinSemanticEngineV3 {
    ...
    pub fn load(&mut self, dataset: &Dataset) {
        ...
        for u in &dataset.users {
            ...
            // Map semantic terms to DAG IDs if not present
            ...
            // Directly index 64-bit canonical DAG IDs
...
fn main() {
    ...
    if args.len() < 4 {
        eprintln!("Usage: rust_engine_v3 <mode> <dataset_json> <workload_json> [out_json]");
        std::process::exit(1);
    }
    ...
    let mut lin = LinSemanticEngineV3::new();
