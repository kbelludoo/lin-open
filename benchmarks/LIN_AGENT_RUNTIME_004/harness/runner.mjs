...
import { BeamAgent } from '../agent/beam_agent.mjs';
...
export async function runBeamBenchmark() {
  console.log("================================================================================");
  console.log("   LIN-AGENT-RUNTIME-004: BEAM SEARCH & HYPOTHESIS RECOVERY (4 ARMS)           ");
  console.log("================================================================================");

  const tasks = JSON.parse(fs.readFileSync(TASKS_FILE, 'utf8'));

  const arms = [
    { id: 'RAW', name: 'Arm A: Raw Model (Zero-State Guessing)' },
    { id: 'CHAT_HISTORY', name: 'Arm B: Chat History Buffer' },
    { id: 'LIN_GREEDY', name: 'Arm C1: LIN Greedy (Single-Node)' },
    { id: 'LIN_BEAM', name: 'Arm C2: LIN Semantic Beam Search + Hypothesis Ledger' }
  ];

  const agent = new BeamAgent();
  ...
  for (const arm of arms) {
    ...
    for (const task of tasks) {
      ...
      console.log(`  [${task.task_id}] ${task.title.slice(0, 35)}... -> Success: ${res.success} (Init: ${res.initialSuccess}, Rec: ${res.recoveredSuccess}) | Localized: ${res.rootCauseFound} | Wrong Edits: ${res.wrongFileEdits} | Tokens: ${res.promptTokens}`);
      taskLogs.push(res);
    }
    ...
    console.log(`\n>>> [${arm.id} Summary] Success: ${(taskSuccessRate * 100).toFixed(1)}% | Localized: ${(localizationRate * 100).toFixed(1)}% | Recovery: ${(recoveryRate * 100).toFixed(1)}% | Wrong Edits: ${totalWrongFileEdits} | Avg Tokens: ${avgTokens.toFixed(0)}`);
  }

  const outPath = path.join(ROOT, 'results/LIN_AGENT_RUNTIME_004_REPORT.json');
  fs.writeFileSync(outPath, JSON.stringify(armSummaries, null, 2));
  console.log(`\n[+] RUNTIME-004 Benchmark Complete! Final Report saved to: ${outPath}`);
  return armSummaries;
}
