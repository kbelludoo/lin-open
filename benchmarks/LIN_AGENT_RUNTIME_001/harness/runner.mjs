...
import { LinAgent } from '../agent/lin_agent.mjs';
...
const TASKS_FILE = path.resolve(ROOT, '../LIN_REAL_AGENT_002/tasks/tasks.json');
...
function initializeSubstrate(agent) {
  ...
  // 1. Register Modules & Symbols with Full HashCons Identity
...
export async function runAgentBenchmark() {
  console.log("================================================================================");
  console.log("   LIN-AGENT-RUNTIME-001: FULL CLOSED-LOOP AGENT BENCHMARK (10 TASKS x 3 ARMS)  ");
  console.log("================================================================================");
  ...
  const groups = [
    ...
    { id: 'CHAT_HISTORY', name: 'Group B: Model + Textual Chat Buffer' },
  ...
  for (const grp of groups) {
    ...
    for (const task of tasks) {
      ...
      console.log(`    -> Success: ${res.success} (Initial: ${res.initialSuccess}, Recovered: ${res.recoveredSuccess}) | Attempts: ${res.attempts} | Prompt Tokens: ${res.promptTokens} | Time: ${(res.durationMs / 1000).toFixed(1)}s`);
      taskLogs.push(res);
    }
    ...
    console.log(`\n[${grp.id} Summary] Task Success: ${(taskSuccessRate * 100).toFixed(1)}% | Recovery: ${(recoveryRate * 100).toFixed(1)}% | Avg Tokens: ${avgTokens.toFixed(0)} | Total Time: ${(totalDurationMs / 1000).toFixed(1)}s`);
  }

  // Compute Semantic State Reuse Metric
  ...
  const semanticStateReuseRate = ((avgTokensChat - avgTokensLin) / avgTokensChat) * 100;

  groupSummaries['COMPARATIVE_METRICS'] = {
    semantic_state_reuse_rate: `${semanticStateReuseRate.toFixed(1)}%`,
    token_reduction: `${((avgTokensChat - avgTokensLin) / avgTokensChat * 100).toFixed(1)}%`,
    recovery_gain: `${((groupSummaries['LIN'].recovery_rate - groupSummaries['CHAT_HISTORY'].recovery_rate) * 100).toFixed(1)}%`
  };

  const outPath = path.join(ROOT, 'results/LIN_AGENT_RUNTIME_001_REPORT.json');
  fs.writeFileSync(outPath, JSON.stringify(groupSummaries, null, 2));
  console.log(`\n[+] Benchmark Complete! Final Report saved to ${outPath}`);
