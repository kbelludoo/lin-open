...
import { DepthAgent } from '../agent/depth_agent.mjs';
...
const TASKS_FILE = path.join(ROOT, 'dataset/depth_controlled_tasks.json');
...
export async function runDepthBenchmark() {
  console.log("================================================================================");
  console.log("   LIN-AGENT-RUNTIME-007: CAUSAL DEPTH (D1..D5) & CUMULATIVE RE-RANKING BENCHMARK");
  console.log("================================================================================");
  ...
  const arms = [
    ...
    { id: 'LIN_CUMULATIVE', name: 'Arm C2: LIN Cumulative Dynamic Re-Ranker' }
  ];

  const agent = new DepthAgent();
  ...
  const outPath = path.join(ROOT, 'results/LIN_AGENT_RUNTIME_007_REPORT.json');
  fs.writeFileSync(outPath, JSON.stringify(armSummaries, null, 2));
  console.log(`\n[+] RUNTIME-007 Benchmark Complete! Final Report saved to: ${outPath}`);
  return armSummaries;
}
