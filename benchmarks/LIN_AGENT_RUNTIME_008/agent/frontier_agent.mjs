// LIN Causal Frontier Closed-Loop Debugging Agent with Stream:false & Failover
...
import { CumulativeStateReRanker } from '../../LIN_AGENT_RUNTIME_007/runtime/cumulative_state_reranker.mjs';
import { CausalFrontierEngine } from '../runtime/causal_frontier_engine.mjs';
...
export class FrontierAgent {
  constructor({
    ...
    this.cumulativeReRanker = new CumulativeStateReRanker(this.substrate, this.ledger);
    this.frontierEngine = new CausalFrontierEngine(this.substrate, this.ledger);
  }
  ...
  async runTask({ task, repoDir, arm = 'LIN_FRONTIER', maxAttempts = 3 }) {
    this.cumulativeReRanker.resetTaskState();
    this.frontierEngine.reset();
    ...
    this.cumulativeReRanker.initializeBaseScores(entrySym, task.test_assertion_error);
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (arm === 'RAW') {
        ...
        const candidates = this.cumulativeReRanker.getRankedCandidates();
      ...
      } else if (arm === 'LIN_FRONTIER') {
        activeCandidate = this.frontierEngine.expandFrontier(entrySym, attempt);
        const causalNarrative = this.frontierEngine.generateCausalNarrative(activeCandidate);
        const focused = this.substrate.generateFocusedContext(activeCandidate.symbol);

        prompt = `${causalNarrative}\n`;
        prompt += `* Declared Effect: *(${focused.effect})\n`;
      ...
      if (testRes.passed) {
        ...
        if (activeCandidate) {
          this.cumulativeReRanker.updateCumulativeStateOnRefutation(activeCandidate.symbol);
          this.frontierEngine.recordRefutation(activeCandidate.symbol, testRes.output);
        }
      }
    }
