...
import { FrontierAgent } from '../agent/frontier_agent.mjs';
...
const TASKS_FILE = path.resolve(ROOT, '../LIN_AGENT_RUNTIME_007/dataset/depth_controlled_tasks.json');
...
export async function runFrontierBenchmark() {
  console.log("================================================================================");
  console.log("   LIN-AGENT-RUNTIME-008: CAUSAL FRONTIER EXPANSION (D1..D5) BENCHMARK          ");
  console.log("================================================================================");
  ...
  const arms = [
    ...
    { id: 'LIN_CUMULATIVE', name: 'Arm C1: LIN Cumulative (Passive Boost - 007)' },
    { id: 'LIN_FRONTIER', name: 'Arm C2: LIN Causal Frontier Expansion' }
  ];
  ...
  for (const arm of arms) {
    ...
    const depthStats = { 1: { total: 0, localized: 0, success: 0 }, 2: { total: 0, localized: 0, success: 0 }, 3: { total: 0, localized: 0, success: 0 }, 4: { total: 0, localized: 0, success: 0 }, 5: { total: 0, localized: 0, success: 0 } };
    const taskLogs = [];
    ...
    for (const task of tasks) {
      ...
      depthStats[task.depth].total++;
      if (res.rootCauseFound) depthStats[task.depth].localized++;
      if (res.success) depthStats[task.depth].success++;

      console.log(`  [${task.task_id} (D${task.depth})] ${task.title.slice(0, 32)}... -> Success: ${res.success} (Init: ${res.initialSuccess}, Rec: ${res.recoveredSuccess}) | Localized: ${res.rootCauseFound} | Wrong Edits: ${res.wrongFileEdits} | Tokens: ${res.promptTokens} | Time: ${(res.durationMs / 1000).toFixed(1)}s`);
      taskLogs.push(res);
    }
    ...
    const depthCurve = {};
    for (let d = 1; d <= 5; d++) {
      const st = depthStats[d];
      depthCurve[`D${d}`] = {
        total: st.total,
        localization_accuracy: st.total > 0 ? `${((st.localized / st.total) * 100).toFixed(1)}%` : '0%',
        success_rate: st.total > 0 ? `${((st.success / st.total) * 100).toFixed(1)}%` : '0%'
      };
    }
    ...
    const summary = {
      ...
      depth_localization_curve: depthCurve,
      tasks: taskLogs
    };
  ...
  const outPath = path.join(ROOT, 'results/LIN_AGENT_RUNTIME_008_REPORT.json');
  fs.writeFileSync(outPath, JSON.stringify(armSummaries, null, 2));
  console.log(`\n[+] RUNTIME-008 Benchmark Complete! Final Report saved to: ${outPath}`);
  return armSummaries;
}

runFrontierBenchmark();
