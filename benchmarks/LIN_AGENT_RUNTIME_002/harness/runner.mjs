...
import { LinAgent } from '../../LIN_AGENT_RUNTIME_001/agent/lin_agent.mjs';
...
const TASKS_FILE = path.join(ROOT, 'dataset/hard_tasks.json');
...
export async function runHardBenchmark() {
  console.log("================================================================================");
  console.log("   LIN-AGENT-RUNTIME-002: 50 HARD REAL TASKS & FAILURE RECOVERY BENCHMARK       ");
  console.log("================================================================================");

  const tasks = JSON.parse(fs.readFileSync(TASKS_FILE, 'utf8'));

  const groups = [
    { id: 'RAW', name: 'Group A: Raw Model (Zero-State Baseline)' },
    { id: 'CHAT_HISTORY', name: 'Group B: Model + Textual Chat History Buffer' },
    { id: 'LIN', name: 'Group C: Model + LIN Semantic Agent Runtime' }
  ];

  const agent = new LinAgent();
  ...
  for (const grp of groups) {
    ...
    for (const task of tasks) {
      ...
      content = content.replace(task.bug_find, task.bug_replace);
      fs.writeFileSync(targetFilePath, content, 'utf8');

      // Execute closed loop with up to 3 attempts
      ...
      totalRejections += res.verificationRejections;

      console.log(`  [${task.task_id}] ${task.symbol} -> Success: ${res.success} (Init: ${res.initialSuccess}, Rec: ${res.recoveredSuccess}) | Attempts: ${res.attempts} | Tokens: ${res.promptTokens} | Time: ${(res.durationMs / 1000).toFixed(1)}s`);
      taskLogs.push(res);
    }
    ...
    const summary = {
      ...
      verification_rejections: totalRejections,
      tasks: taskLogs
    };

    groupSummaries[grp.id] = summary;
    console.log(`\n>>> [${grp.id} Final] Success: ${(taskSuccessRate * 100).toFixed(1)}% | Recovery: ${(recoveryRate * 100).toFixed(1)}% | Avg Tokens: ${avgTokens.toFixed(0)} | Total Time: ${(totalDurationMs / 1000).toFixed(1)}s`);
  }

  // Differential comparative metrics
  ...
  groupSummaries['COMPARATIVE_ANALYSIS'] = {
    ...
    chat_recovery_rate: `${(groupSummaries['CHAT_HISTORY'].recovery_rate * 100).toFixed(1)}%`,
    lin_recovery_rate: `${(groupSummaries['LIN'].recovery_rate * 100).toFixed(1)}%`,
    lin_token_reduction_vs_chat: `${tokenReduction.toFixed(1)}%`,
    lin_speedup_vs_raw: `${((groupSummaries['RAW'].total_duration_s - groupSummaries['LIN'].total_duration_s) / groupSummaries['RAW'].total_duration_s * 100).toFixed(1)}%`
  };

  const outPath = path.join(ROOT, 'results/LIN_AGENT_RUNTIME_002_REPORT.json');
  fs.writeFileSync(outPath, JSON.stringify(groupSummaries, null, 2));
  console.log(`\n[+] Full 50-Task Benchmark Complete! Report saved to: ${outPath}`);
