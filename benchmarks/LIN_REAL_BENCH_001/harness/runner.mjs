...
import { RealAblationAgent } from '../agent/real_ablation_agent.mjs';

const __filename = fileURLToPath(import.meta.url);
const ROOT = path.resolve(path.dirname(__filename), '..');
const TASKS_FILE = path.join(ROOT, 'dataset/historical_bugs.json');

export async function runRealAblationBenchmark() {
  console.log("================================================================================");
  console.log("   LIN-REAL-BENCH-001: REAL-WORLD OPEN-SOURCE BUGS & 7-STAGE ABLATION           ");
  console.log("================================================================================");

  const tasks = JSON.parse(fs.readFileSync(TASKS_FILE, 'utf8'));

  const ablationArms = [
    { id: 'ARM_A_RAW', name: 'Arm A: Raw LLM Text Retrieval' },
    { id: 'ARM_B_CHAT', name: 'Arm B: Chat History Buffer' },
    { id: 'ARM_C_HASHCONS', name: 'Arm C: HashCons Memory Indexing' },
    { id: 'ARM_D_DAG', name: 'Arm D: HashCons + Dependency DAG' },
    { id: 'ARM_E_CONTRACTS_EFFECTS', name: 'Arm E: DAG + Contracts (%) + Effects (*)' },
    { id: 'ARM_F_TRAUMA', name: 'Arm F: Contracts + Effects + Trauma Ledger' },
    { id: 'ARM_G_FULL_LIN', name: 'Arm G: Full LIN Causal Frontier Navigation' }
  ];

  const agent = new RealAblationAgent();
  const armSummaries = {};

  for (const arm of ablationArms) {
    console.log(`\n================================================================================`);
    console.log(`>>> Executing ${arm.name} on Real-World Repositories...`);
    console.log(`================================================================================`);
    ...
    for (const task of tasks) {
      ...
      const res = await agent.runAblationTask({
        task,
        repoDir,
        arm: arm.id,
        maxAttempts: 3
    ...
    console.log(`\n>>> [${arm.id} Summary] Success: ${(taskSuccessRate * 100).toFixed(1)}% | Localized: ${(localizationRate * 100).toFixed(1)}% | Recovery: ${(recoveryRate * 100).toFixed(1)}% | Avg Tokens: ${avgTokens.toFixed(0)} | Total Time: ${(totalDurationMs / 1000).toFixed(1)}s`);
  }

  const outPath = path.join(ROOT, 'results/LIN_REAL_BENCH_001_REPORT.json');
  fs.writeFileSync(outPath, JSON.stringify(armSummaries, null, 2));
  console.log(`\n[+] LIN-REAL-BENCH-001 Complete! Final Report saved to: ${outPath}`);
  return armSummaries;
}

runRealAblationBenchmark();
