...
pub fn main() !void {
    ...
    if (argEq(cmd, "verify-certificate") or argEq(cmd, "verify-cert")) {
        ...
        const compiler_match = claimed_compiler != null and
            std.mem.eql(u8, claimed_compiler.?, std.fmt.fmtSliceHexLower(&compiler_digest));
        const corpus_match = claimed_corpus != null and
            std.mem.eql(u8, claimed_corpus.?, std.fmt.fmtSliceHexLower(&corpus_digest));
        const ledger_match = claimed_ledger != null and
            std.mem.eql(u8, claimed_ledger.?, std.fmt.fmtSliceHexLower(&ledger_digest));
        const certificate_match = cert_id_claim.len > "sha256:".len and
            std.mem.eql(u8, cert_id_claim[0.."sha256:".len], "sha256:") and
            std.mem.eql(u8, cert_id_claim["sha256:".len..], std.fmt.fmtSliceHexLower(&cert_digest));
    ...
    if (argEq(cmd, "cert")) {
        ...
        if (argEq(sub, "build")) {
            ...
            for (corpus_targets) |t| {
                if (std.fs.cwd().openFile(t.file, .{})) |pf| {
                    pf.close();
                    corpus_present += 1;
                } else |_| {}
            }
