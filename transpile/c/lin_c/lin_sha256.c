/*
 * lin_sha256.c — SHA-256 (FIPS 180-4), self-contained, no libc beyond string.h.
 */
...
static void lin_sha256_transform(LinSha256 *ctx, const uint8_t block[64]) {
    uint32_t w[64];
    for (int i = 0; i < 16; i++) {
...
void lin_sha256_update(LinSha256 *ctx, const void *data, size_t len) {
    ...
    while (len > 0) {
        const size_t take = (64u - ctx->buf_len < len) ? (64u - ctx->buf_len) : len;
        memcpy(ctx->buf + ctx->buf_len, p, take);
        ctx->buf_len += take;
        p += take;
        len -= take;
        if (ctx->buf_len == 64u) {
            lin_sha256_transform(ctx, ctx->buf);
            ctx->buf_len = 0;
        }
    }
}
...
void lin_sha256_final(LinSha256 *ctx, uint8_t out[32]) {
    ...
    for (int i = 0; i < 8; i++) ctx->buf[56 + i] = (uint8_t)(bitlen >> (56 - 8 * i));
    lin_sha256_transform(ctx, ctx->buf);
