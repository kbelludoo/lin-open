...
def replace_block(text, start_str, end_str, new_code):
    start = text.find(start_str)
    if start == -1: raise Exception("start not found: " + start_str)
    end = text.find(end_str, start)
    if end == -1: raise Exception("end not found: " + end_str)
    end += len(end_str)
    return text[:start] + new_code + text[end:]

# 1. Replace js_equals
js_equals_old_start = "fn js_equals(v1: &Value, v2: &Value) -> bool {"
js_equals_old_end = "}\n\nfn is_truthy"
...
text = replace_block(text, js_equals_old_start, js_equals_old_end, js_equals_new)

# 2. Replace js_rel_lt
js_rel_lt_old_start = "fn js_rel_lt(a: &Value, b: &Value) -> bool {"
js_rel_lt_old_end = "}\n\n// JS + operator:"
...
text = replace_block(text, js_rel_lt_old_start, js_rel_lt_old_end, js_rel_lt_new)

# 3. Replace eval_expr
eval_expr_old_start = "pub fn eval_expr(expr: &str, scope: &mut Scope, module: &LinModule) -> Value {"
eval_expr_old_end = "}\n\nfn js_equals"
...
text = replace_block(text, eval_expr_old_start, eval_expr_old_end, eval_expr_new)

# 4. Replace find_binary_op
find_binary_op_old_start = "fn find_binary_op(s: &str, op: &str) -> Option<usize> {"
find_binary_op_old_end = "}\n\npub fn execute_statement_block"
...
text = replace_block(text, find_binary_op_old_start, find_binary_op_old_end, find_binary_op_new)
