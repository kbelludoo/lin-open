#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Schema 1.8: LIP1 inclusion + all LCR2 and LCR2-full E2E leaves; Java still compiled here."""
...
VERIFY_INC_PL = ROOT / "examples" / "u512_coprocessor" / "verify_u512_inclusion.pl"
...
def make_lip1(lnr1: list[bytes], lnr1_root: str, lcr2: list[bytes], lcr2_root: str,
    ...
    for i, leaf in enumerate(full_leaves):
        cls = leaf.get("class") or f"full{i}"
        name = "PHANTOM_APPROVE" if cls == "PHANTOM_APPROVE" else f"FULL_{cls}_{i}"
        proofs.append(make_proof("LCR2_FULL", name, i, full_recs))
    ...
    n_fex = sum(1 for p in proofs if p["name"].startswith("FULL_EXACT_INPUT_"))
    n_fov = sum(1 for p in proofs if p["name"].startswith("FULL_OVERPAID_INPUT_"))
    n_fz = sum(1 for p in proofs if p["name"].startswith("FULL_GUARD_ZERO_"))
    n_ffee = sum(1 for p in proofs if p["name"].startswith("FULL_GUARD_FEE_OVERFLOW_"))
    n_fph = sum(1 for p in proofs if p["name"] == "PHANTOM_APPROVE" and p["tree"] == "LCR2_FULL")
    ...
    ok = ok and n_fex >= 3 and n_fov >= 3 and n_fz >= 1 and n_ffee >= 1 and n_fph >= 1
    ...
    phf = next(p for p in proofs if p["tree"] == "LCR2_FULL" and p["name"] == "PHANTOM_APPROVE")
    leaf_d3, _node_d3, _sz3, tid3 = TREES["LCR2_FULL"]
    rec3 = bytes.fromhex(phf["leaf_hex"])
    sibs3 = [bytes.fromhex(s) for s in phf["siblings"]]
    root3 = bytes.fromhex(phf["root"])
    leaf_h3 = hashlib.sha256(leaf_d3 + rec3).digest()
    blob_phf = Path("/tmp/u512_lip1_lcr2_full_phantom.bin")
    blob_phf.write_bytes(pack_lip1(tid3, phf["index"], leaf_h3, root3, sibs3, phf["sides"]))
    c_phf = run([str(aud), "--lip1", str(blob_phf)])
    c_phf_ok = c_phf.returncode == 0 and "LIP1_PASS" in c_phf.stdout and phf["root"] in c_phf.stdout
    tamp3 = bytearray(blob_phf.read_bytes())
    tamp3[80 + 4] ^= 1
    tpath3 = Path("/tmp/u512_lip1_lcr2_full_tamper.bin")
    tpath3.write_bytes(bytes(tamp3))
    t3 = run([str(aud), "--lip1", str(tpath3)])
    t3_ok = t3.returncode != 0
    return {
        "ok": ok, "c11": c_ok and t_ok, "c11_lcr2": c_ex_ok and t2_ok,
        "c11_full": c_phf_ok and t3_ok,
        "detail": (f"n={len(proofs)} lcr2={n_ex}+{n_ov}+{n_z}+{n_ovf} "
                   f"full={n_fex}+{n_fov}+{n_fz}+{n_ffee}+ph{n_fph} names={sorted(need & names)}"),
        "c11_detail": (c.stdout or "")[:160],
        "c11_lcr2_detail": (c_ex.stdout or "")[:160],
        "c11_full_detail": (c_phf.stdout or "")[:160],
        "proofs": [{k: v for k, v in p.items() if k != "ok"} for p in proofs],
        "n": len(proofs), "blob": blob, "blob_lcr2": blob_ex, "blob_full": blob_phf,
    }
...
def inclusion_auditor_tuples(evid: Path):
    ...
    ipl = run(["perl", str(VERIFY_INC_PL), str(evid)], timeout=30)
    need = ("lip1_match", "lcr2_inclusion", "lcr2_full_inclusion")
    return [
        ("standalone_inclusion_py", ip.returncode == 0 and all(k in ip.stdout for k in need),
         ip.stdout.strip()[:220]),
        ("standalone_inclusion_js", ij.returncode == 0 and all(k in ij.stdout for k in need),
         ij.stdout.strip()[:220]),
        ("standalone_inclusion_pl", ipl.returncode == 0 and all(k in ipl.stdout for k in need),
         ipl.stdout.strip()[:220]),
    ]
