#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Schema 1.6: Java BigInteger consensus + LIP1 Merkle inclusion receipts."""
...
def make_lip1(lnr1: list[bytes], lnr1_root: str, lcr2: list[bytes], lcr2_root: str,
    ...
    ov_i = next(i for i, L in enumerate(lcr_leaves) if L.get("class") == "GUARD_OVERFLOW")
    proofs.append(make_proof("LCR2", "GUARD_OVERFLOW", ov_i, lcr2))
    ...
    need = {"phantom_997_rout_1997", "wrap_borrow_2_x_2_255_div_umax", "two255_x2_div3",
            "max_sq", "GUARD_OVERFLOW", "PHANTOM_APPROVE", "boundary_2_256_ge_umax"}
    ok = ok and need <= names
    ...
    t_ok = t.returncode != 0
    return {
        "ok": ok, "c11": c_ok and t_ok,
        "detail": f"n={len(proofs)} names={sorted(need & names)}",
        "c11_detail": (c.stdout or "")[:160],
        "proofs": [{k: v for k, v in p.items() if k != "ok"} for p in proofs],
        "n": len(proofs), "blob": blob,
    }
