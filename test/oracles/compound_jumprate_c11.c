/*
 * Independent C11 oracle for Compound JumpRateModel V2 math (LINVM-1 i64).
 *
 ...
 *
 * Quotient that does not fit nonnegative i64 fail-closes to 0 (same as LIN).
...
static int selftest(void) {
    ...
    s50a = supply_rate(BASE_WAD, BASE_WAD, 0, 0, mb_a, jb_a, kink, 100000000000000000ULL, muldiv_i128);
    s50b = supply_rate(BASE_WAD, BASE_WAD, 0, 0, mb_b, jb_b, kink, 100000000000000000ULL, muldiv_limbs);
    if (r50a != r50b || r90a != r90b || s50a != s50b) fail++;
    if (r90a < r50a) fail++;
    if (s50a == 0 || s50a >= r50a) fail++;
...
static void print_muldiv_id(uint64_t a, uint64_t b, uint64_t d) {
    __uint128_t prod, qf, rf;
    uint64_t qout;
    if (d == 0) {
        printf("0 0 1\n");
        return;
    ...
    qf = prod / d;
    rf = prod % d;
    qout = (qf > (uint64_t)INT64_MAX) ? 0 : (uint64_t)qf;
    printf("%" PRIu64 " %" PRIu64 " %d\n", qout, (uint64_t)rf, qf > (uint64_t)INT64_MAX);
}
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
                "usage: %s selftest | muldiv a b d | muldiv-id a b d | util c b r | borrow c b r base mult jump kink | supply ... rf\n",
                argv[0]);
        return 2;
    }
    cmd = argv[1];
    ...
    if (strcmp(cmd, "muldiv-id") == 0 && argc == 5) {
        print_muldiv_id(strtoull(argv[2], NULL, 10),
