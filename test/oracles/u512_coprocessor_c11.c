/*
 * Independent C11 oracle for LIN u512 coprocessor (not TCB).
 * 16-bit schoolbook must match 64-bit __int128 limbs; Python is third.
...
static void be32_to_l16(const uint8_t be[32], uint16_t l[16]) {
    for (int i = 0; i < 16; i++) {
        int o = 30 - 2 * i;
        l[i] = (uint16_t)(((uint16_t)be[o] << 8) | be[o + 1]);
    }
}
...
static int muldiv64(const uint64_t a[4], const uint64_t b[4], const uint64_t d[4],
    ...
    st = div64(p, d, q, rem);
    if (st != 1) return st;
    if (rem[4] | rem[5] | rem[6] | rem[7]) return -2;
