 ...
 *
 * Path A: 64-bit schoolbook using GCC/Clang unsigned __int128 (64x64->128).
 * Path B: 16-bit limbs matching src/lin_u512_coprocessor.lin.
 * Consensus A==B is required before any LIN comparison.
 * Not part of the LIN compiler TCB.
 */
#include <inttypes.h>
#include <stdint.h>
#include <stdio.h>
...
static int ge16_words(const uint64_t a[4], const uint64_t b[4]) {
    uint64_t ax[8], bx[8];
    z8(ax); z8(bx);
    memcpy(ax, a, 32); memcpy(bx, b, 32);
    return ge8(ax, bx);
}
...
static int selftest(void) {
    ...
    for (i = 0; i < (int)(sizeof(vec) / sizeof(vec[0])); i++) {
        ...
        if (!parse_hex_u256(vec[i].a, a) || !parse_hex_u256(vec[i].b, b) ||
            !parse_hex_u256(vec[i].d, d)) {
            fprintf(stderr, "parse fail i=%d\n", i); return 1;
        }
        muldiv_wide(a, b, d, &w);
        muldiv_limbs(a, b, d, &l);
        if (!res_eq(&w, &l)) {
            fprintf(stderr, "mismatch curated i=%d wide.status=%d limbs.status=%d\n",
                    i, w.status, l.status);
            fail++;
        }
    }
    for (i = 0; i < 256; i++) {
        ...
        muldiv_limbs(a, b, d, &l);
        if (!res_eq(&w, &l)) {
            fprintf(stderr, "mismatch random i=%d\n", i);
            fail++;
            break;
        }
    }
    {
        uint64_t one[4] = {1, 0, 0, 0}, two[4] = {2, 0, 0, 0};
        if (ge16_words(two, one) != 1 || ge16_words(one, two) != 0) fail++;
    }
    if (fail) {
...
int main(int argc, char **argv) {
    ...
    int use_limbs = 0;
    if (argc < 2) {
        fprintf(stderr, "usage: %s selftest | muldiv a b d [--limbs] | mul a b [--limbs] | ge a512lo a512hi b512lo b512hi\n",
                argv[0]);
        return 2;
    }
    cmd = argv[1];
    ...
    if (strcmp(cmd, "muldiv") == 0 && argc == 5) {
        ...
        if (!res_eq(&l, &w)) {
            fprintf(stderr, "internal wide/limbs diverge\n"); return 1;
        }
        emit(&l);
        return 0;
    }
    if (strcmp(cmd, "mul") == 0 && argc == 4) {
        ...
        if (memcmp(w.n, l.n, 64) != 0) {
            fprintf(stderr, "mul wide vs limbs diverge\n"); return 1;
        }
        print_u512("n", w.n);
        return 0;
    }
