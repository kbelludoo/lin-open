/*
 * C11 auditor for LNR1 (240B), LCR2 (208B), LGE1, LRI1, LIP1 receipts. Zero LIN VM.
...
static int domain_of(uint8_t tree, const uint8_t **dl, size_t *dll,
                     const uint8_t **dn, size_t *dnl) {
    if (tree == 1) {
        *dl = DOM_LNR_L; *dll = sizeof(DOM_LNR_L) - 1;
        *dn = DOM_LNR_N; *dnl = sizeof(DOM_LNR_N) - 1; return 0;
    }
    if (tree == 2 || tree == 3) {
        *dl = DOM_LCR_L; *dll = sizeof(DOM_LCR_L) - 1;
        *dn = DOM_LCR_N; *dnl = sizeof(DOM_LCR_N) - 1; return 0;
    }
    if (tree == 4) {
        *dl = DOM_GE_L; *dll = sizeof(DOM_GE_L) - 1;
        *dn = DOM_GE_N; *dnl = sizeof(DOM_GE_N) - 1; return 0;
    }
    if (tree == 5) {
        *dl = DOM_RI_L; *dll = sizeof(DOM_RI_L) - 1;
        *dn = DOM_RI_N; *dnl = sizeof(DOM_RI_N) - 1; return 0;
    }
    return -1;
}
...
static int verify_lip1(const char *path) {
    uint8_t *buf = NULL;
    size_t n = 0;
    const uint8_t *dl, *dn;
    size_t dll, dnl;
    ...
    char hex[65];
    if (load_file(path, &buf, &n) != 0) return 1;
    if (n < 80 || memcmp(buf, "LIP1", 4) != 0 || buf[4] != 1) { free(buf); return 1; }
    if (domain_of(buf[5], &dl, &dll, &dn, &dnl) != 0) { free(buf); return 1; }
    nsib = u32le(buf + 12);
    if (n != 80 + 36u * nsib) { free(buf); return 1; }
    memcpy(cur, buf + 16, 32);
    for (i = 0; i < nsib; i++) {
        const uint8_t *slot = buf + 80 + 36u * i;
        uint8_t side = slot[0], out[32];
        const uint8_t *sib = slot + 4;
        if (side == 0) h_node(dn, dnl, cur, sib, out);
        else if (side == 1) h_node(dn, dnl, sib, cur, out);
        else { free(buf); return 1; }
        memcpy(cur, out, 32);
    }
    if (memcmp(cur, buf + 48, 32) != 0) {
        fprintf(stderr, "FAIL LIP1 root mismatch\n");
        free(buf);
        return 1;
    }
    lin_sha256_hex(cur, hex);
    printf("LIP1_PASS root=%s nsib=%u\n", hex, nsib);
    free(buf);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    if (argc >= 3 && strcmp(argv[1], "--lip1") == 0)
        return verify_lip1(argv[2]);
    fprintf(stderr,
