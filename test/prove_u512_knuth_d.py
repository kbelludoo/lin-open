#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Schema 1.12: Knuth Algorithm D (normalized long division, not restoring)."""
...
KD_SRC = ROOT / "test" / "oracles" / "u512_knuth_d.py"
PY = sys.executable


def _cmd() -> list[str]:
    return [PY, str(KD_SRC)]


def kd_selftest() -> tuple[bool, str]:
    p = run(_cmd() + ["--self-test"], timeout=30)
    return p.returncode == 0 and "SELFTEST_PASS" in p.stdout, p.stdout.strip()


def kd_batch(lines: str, expect: list) -> tuple[bool, int, str]:
...
def kd_ge_batch() -> tuple[bool, str]:
...
def _bind(leaves: list[dict], classes: tuple[str, ...], min_n: int, need_ph: bool) -> tuple[bool, str]:
    ...
    ok, n, err = kd_batch("".join(lines), expect)
    return ok, f"n={n} {err}"


def kd_lcr2_bind(leaves: list[dict]) -> tuple[bool, str]:
    return _bind(leaves, ("EXACT_INPUT", "OVERPAID_INPUT"), 6, False)


def kd_lcr2_full_bind(leaves: list[dict]) -> tuple[bool, str]:
    return _bind(leaves, ("EXACT_INPUT", "OVERPAID_INPUT", "PHANTOM_APPROVE"), 7, True)


def kd_lri1_identity(leaves: list[dict]) -> tuple[bool, str]:
    lines, expect = [], []
    for leaf in leaves:
...
def kd_check_tuples(lines: str, expect: list, match: bool):
    st, stout = kd_selftest()
    ok, n, berr = kd_batch(lines, expect) if st else (False, 0, "no selftest")
    ge_ok, ge_err = kd_ge_batch() if st else (False, "no selftest")
    info = {
        "selftest": st, "batch": ok, "n": n, "ge": ge_ok,
        "src": sha256_file(KD_SRC),
    }
    return info, [
        ("knuth_d_selftest", st, stout),
        ("knuth_d_python_c11_full_range", ok and match, f"n={n} {berr}"),
        ("knuth_d_ge512", ge_ok, ge_err),
    ]
