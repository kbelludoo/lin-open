#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Schema 1.14: Karatsuba 256x256->512 + Barrett 512/256 (not restoring of n)."""
...
KS_SRC = ROOT / "test" / "oracles" / "u512_karatsuba_barrett.py"
PY = sys.executable


def _cmd() -> list[str]:
    return [PY, str(KS_SRC)]


def ks_selftest() -> tuple[bool, str]:
    p = run(_cmd() + ["--self-test"], timeout=30)
    return p.returncode == 0 and "SELFTEST_PASS" in p.stdout, p.stdout.strip()


def ks_batch(lines: str, expect: list) -> tuple[bool, int, str]:
...
def ks_ge_batch() -> tuple[bool, str]:
...
def _bind(leaves: list[dict], classes: tuple[str, ...], min_n: int, need_ph: bool) -> tuple[bool, str]:
    ...
    ok, n, err = ks_batch("".join(lines), expect)
    return ok, f"n={n} {err}"


def ks_lcr2_bind(leaves: list[dict]) -> tuple[bool, str]:
    return _bind(leaves, ("EXACT_INPUT", "OVERPAID_INPUT"), 6, False)


def ks_lcr2_full_bind(leaves: list[dict]) -> tuple[bool, str]:
    return _bind(leaves, ("EXACT_INPUT", "OVERPAID_INPUT", "PHANTOM_APPROVE"), 7, True)


def ks_lri1_identity(leaves: list[dict]) -> tuple[bool, str]:
    lines, expect = [], []
    for leaf in leaves:
...
def ks_check_tuples(lines: str, expect: list, match: bool):
    st, stout = ks_selftest()
    ok, n, berr = ks_batch(lines, expect) if st else (False, 0, "no selftest")
    ge_ok, ge_err = ks_ge_batch() if st else (False, "no selftest")
    info = {
        "selftest": st, "batch": ok, "n": n, "ge": ge_ok,
        "src": sha256_file(KS_SRC),
    }
    return info, [
        ("karatsuba_barrett_selftest", st, stout),
        ("karatsuba_python_c11_full_range", ok and match, f"n={n} {berr}"),
        ("karatsuba_ge512", ge_ok, ge_err),
    ]
