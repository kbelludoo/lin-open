...
Proves this run (no Zig required):
  * 256x256->512 mul carry + leftover fail-closed
  * 512/256 restoring div with 32-limb remainder, 512-bit ge
  * remainder identity q*d+r == n and r<d vs Python int
  * C11 16-bit schoolbook == C11 64-bit __int128 limbs == Python
  * LNR1 Merkle tri-auditor (Python hashlib, Node crypto, C11 lin_sha256)
    rejects a 1-bit leaf tamper
  * LCR2 slice: settle_u256_word vs ref_amount_out on 1 EXACT + 1 OVERPAID
    with real steps; guards -1/-2

Does NOT prove: FullMath CRT/mulmod assembly; a deployed pool; the 2000/157
campaign; that LIN replaces Uniswap/EVM.
...
def run(cmd: list[str], inp: str | None = None, timeout: int = 120) -> subprocess.CompletedProcess[str]:
    return subprocess.run(cmd, input=inp, capture_output=True, text=True, timeout=timeout, check=False)
...
def lin(fn: str, *args: int, src: Path = LIN) -> tuple[int, int]:
    p = run([str(C0), "vm", str(src), fn, *[str(a) for a in args]], timeout=60)
    if p.returncode != 0:
...
def main() -> int:
    ...
    ok("c11_oracle_compile", cc1.returncode == 0, cc1.stderr[-200:] if cc1.returncode else "")
    cc2 = run(["gcc", "-O2", "-Wall", "-Wextra", "-std=c11", f"-I{ROOT}/transpile/c/lin_c",
               "-o", str(aud), str(AUD_SRC), str(SHA_C)])
    ok("c11_auditor_compile", cc2.returncode == 0, cc2.stderr[-200:] if cc2.returncode else "")
    ...
    ok("lin_ge_512", ge1 == 1 and ge0 == 0, f"ge={ge1},{ge0}")
    ...
    ok("lnr1_tri_auditor", py_root == c_root and n_root == py_root,
    ...
    for rec in (exact, over):
        ...
        st_lin, s_st = lin("settle_u256_word", *aw, *riw, *row, -1, src=U256)
        ws, s_w0 = [], 0
        for i in range(4):
        ...
        cls = rec["class"]
        match_ref = st_lin == rst and (rst != 1 or got == ref)
        if cls == "EXACT_INPUT":
            match_on = got == onchain
        else:
            match_on = True
        if not match_ref or s_w0 == 0 or not match_on:
            e2e_ok = False
    ...
    ok("lcr2_exact_overpaid_steps", e2e_ok and g_st == -1 and g_ov == -2,
    ...
    ok("image_encode", im.returncode == 0 and img_p.exists(), sha256_file(img_p)[:16] if img_p.exists() else im.stderr[-80:])
