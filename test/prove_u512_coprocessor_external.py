...
def main() -> int:
    ...
    rec_claim(claims, "C11-SELF", "PASS" if st.returncode == 0 and "PASS" in st.stdout else "FAIL",
              "C11 2^255*2/3: product 2^256, q=0x5555 repeating, r=1",
              st.stdout.strip()[:80])
    if st.returncode != 0:
        return 1
    ...
    vr = json.loads(v.stdout) if v.stdout.strip().startswith("{") else {"valid": False, "parse": v.stdout[:200]}
    rec_claim(claims, "AUDITOR", "PASS" if vr.get("valid") and vr.get("self_test_tamper_rejected") else "FAIL",
              "stdlib verifier (no LIN) math+merkle + 1-bit tamper reject",
              (vr.get("merkle_root") or "")[:16])
