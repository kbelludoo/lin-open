...
This run (no Zig):
  ...
  * q*d+r==n vs Python int + C11 16-bit + C11 64-bit __int128 + Node BigInt
    + OpenSSL BIGNUM (dlopen libcrypto.so.3)
  ...
  * LCR2: settle_u256_word vs coprocessor mulDiv vs ref_amount_out
    on 3 EXACT + 3 OVERPAID with real steps; guards -1/-2 published
  * LCR2-full: settle_u512_swap vs ref_amount_out_full including PHANTOM_APPROVE
  * LGE1: eight published 512-bit ge leaves (power-of-two tree) vs Python,
    Node BigInt, OpenSSL BN_ucmp, and LIN u512_ge_word
  * Zero-LIN auditors in Python AND Node recompute four merkles

Does NOT prove FullMath CRT/mulmod, a pool, the 2000/157 campaign,
or that 512-bit math closes the SafeMath-width u256 settler by itself.
...
def main() -> int:
    ...
    evidence = {
        "schema": "LIN_U512_COPROCESSOR_EVIDENCE_1.4",
        "class": "EXPERIMENTAL",
        ...
        "lge1_merkle": ge["root"],
        "audit": "zero-LIN Python and Node verifiers recompute LNR1, LCR2, LCR2-full, and LGE1 merkles",
        "pins": {
    ...
    ok("standalone_receipt_verifier", vr.returncode == 0 and "PASS" in vr.stdout
       ...
       and "lge1_match" in vr.stdout, vr.stdout.strip()[:240])
    js = run([node_bin(), str(VERIFY_JS), str(EVID)], timeout=30)
    ok("standalone_node_verifier", js.returncode == 0 and "PASS" in js.stdout
       and "lcr2_full_match" in js.stdout and "lge1_match" in js.stdout, js.stdout.strip()[:240])
