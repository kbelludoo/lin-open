...
Oracles: Python int, C11 16-bit, C11 __int128, C11 radix-2^32, Node BigInt,
OpenSSL BN, Go math/big, Java BigInteger, GNU MP mpz, Perl Math::BigInt::Calc,
POSIX GNU bc. LNR1+LCR2+LCR2-full+LGE1+LRI1 + LIP1 inclusion (all LCR2 and LCR2-full).
EXPERIMENTAL FullMath width, not CRT, not a pool, not the 2000/157 campaign.
"""
...
def main() -> int:
    ...
    evidence = {
        "schema": "LIN_U512_COPROCESSOR_EVIDENCE_1.9",
        "class": "EXPERIMENTAL",
        ...
        "audit": "zero-LIN Python/Node/Perl recompute five merkles; LIP1 inclusion verifies one leaf without the campaign; packed LCR2 binds to ref_amount_out; packed LCR2-full binds to ref_amount_out_full; POSIX GNU bc checks LRI1 q*d+r==n; C11 radix-2^32 schoolbook agrees on carry",
        "pins": {
    ...
    print("self_gate_steps", s_gate, "class EXPERIMENTAL FullMath-width not CRT not a pool")
