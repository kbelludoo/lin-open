...
This run (no Zig):
  ...
  * LCR2: settle_u256_word vs coprocessor mulDiv vs ref_amount_out
    ...
    as LCR2 leaves so the zero-LIN verifier recomputes that merkle too
  * Node.js BigInt remainder identity (independent of Python and C11)
  * LCR2-full: settle_u512_swap vs ref_amount_out_full (512-bit numerator)
    on 3 EXACT + 3 OVERPAID + PHANTOM_APPROVE + GUARD_ZERO + GUARD_FEE_OVERFLOW
  * Zero-LIN auditors in Python AND Node recompute three merkles
...
def main() -> int:
    ...
    evidence = {
        "schema": "LIN_U512_COPROCESSOR_EVIDENCE_1.3",
        "class": "EXPERIMENTAL",
        ...
        "audit": "zero-LIN Python and Node verifiers recompute LNR1, LCR2, and LCR2-full merkles",
        "pins": {
    ...
    ok("standalone_receipt_verifier", vr.returncode == 0 and "PASS" in vr.stdout
       and "merkle_match" in vr.stdout and "lcr2_full_match" in vr.stdout, vr.stdout.strip()[:200])
    js = run([node_bin(), str(VERIFY_JS), str(EVID)], timeout=30)
    ok("standalone_node_verifier", js.returncode == 0 and "PASS" in js.stdout
       and "lcr2_full_match" in js.stdout, js.stdout.strip()[:200])
