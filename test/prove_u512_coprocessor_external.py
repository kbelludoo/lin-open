#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""External proof: LIN u512 numeric coprocessor vs Python int vs C11 vs Node.

Proves (recomputed this run): 256x256->512 mul, 512-bit ge, restoring
512/256 div, remainder identity q*d+r==n, guards -1/-2, C11 dual schoolbook
== Python int (n=520 batch), LINBC1 on goldens+sample, phantom AMM APPROVE
vs settle_u256_word -2, 1 EXACT + 1 OVERPAID LCR2 with real steps, LNR1
verified by Python hashlib AND Node.js crypto.

Not proved: FullMath CRT/mulmod, deployed pool, 2000/157 campaign, zk.
Class: EXPERIMENTAL. FullMath.sol is pinned as WIDTH spec only.
"""
...
def main() -> int:
    ...
    ver = run([sys.executable, str(VERIFY_PY), "--self-test", str(EVID)])
    rec("AUDITOR-PY", "PASS" if ver.returncode == 0 else "FAIL",
        "verify_u512_receipt.py --self-test + bundle", (ver.stdout or "").strip().split("\n")[0])
