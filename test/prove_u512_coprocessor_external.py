...
Newton inverse, Node BigInt, OpenSSL BN, Go math/big, Java BigInteger, GNU MP
mpz, Perl Math::BigInt::Calc, POSIX GNU bc. LNR1+LCR2+LCR2-full+LGE1+LRI1 +
LIP1 inclusion (all LCR2 and LCR2-full). EXPERIMENTAL FullMath algorithm off-chain,
not the EVM mulmod opcode, not a pool, not the 2000/157 campaign.
"""
...
def main() -> int:
    ...
    evidence = {
        "schema": "LIN_U512_COPROCESSOR_EVIDENCE_1.13",
        "class": "EXPERIMENTAL",
        ...
        "audit": "zero-LIN Python/Node/Perl recompute five merkles; LIP1 inclusion verifies one leaf without the campaign; packed LCR2 binds to ref_amount_out; packed LCR2-full binds to ref_amount_out_full; POSIX GNU bc checks LRI1 q*d+r==n; C11 radix-2^32, rustc radix-2^64, and Python radix-2^8 schoolbooks agree on carry; Knuth Algorithm D (normalized long division, not restoring) agrees on q*d+r==n; FullMath CRT reconstructs the 512-bit product (prod0 wrapping, mm Mersenne rotate-and-add, prod1 from CRT) and Newton inverse divides without restoring; rustc-only auditor recomputes LRI1 Merkle + LCR2 bind; radix-2^8, Knuth-D, and CRT auditors recompute identity + OpenSSL dgst LRI1 Merkle",
        "pins": {
    ...
    print("self_gate_steps", s_gate, "class EXPERIMENTAL FullMath CRT+Newton auditor not EVM mulmod not a pool")
