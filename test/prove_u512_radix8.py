#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Schema 1.11: Python radix-2^8 schoolbook (carry independent of 16/32/64)."""
...
R8_SRC = ROOT / "test" / "oracles" / "u512_radix8.py"
PY = sys.executable


def _cmd() -> list[str]:
    return [PY, str(R8_SRC)]


def r8_selftest() -> tuple[bool, str]:
    p = run(_cmd() + ["--self-test"], timeout=30)
    return p.returncode == 0 and "SELFTEST_PASS" in p.stdout, p.stdout.strip()


def r8_batch(lines: str, expect: list) -> tuple[bool, int, str]:
...
def r8_ge_batch() -> tuple[bool, str]:
...
def _bind(leaves: list[dict], classes: tuple[str, ...], min_n: int, need_ph: bool) -> tuple[bool, str]:
    ...
    ok, n, err = r8_batch("".join(lines), expect)
    return ok, f"n={n} {err}"


def r8_lcr2_bind(leaves: list[dict]) -> tuple[bool, str]:
    return _bind(leaves, ("EXACT_INPUT", "OVERPAID_INPUT"), 6, False)


def r8_lcr2_full_bind(leaves: list[dict]) -> tuple[bool, str]:
    return _bind(leaves, ("EXACT_INPUT", "OVERPAID_INPUT", "PHANTOM_APPROVE"), 7, True)


def r8_lri1_identity(leaves: list[dict]) -> tuple[bool, str]:
    lines, expect = [], []
    for leaf in leaves:
...
def r8_check_tuples(lines: str, expect: list, match: bool):
    st, stout = r8_selftest()
    ok, n, berr = r8_batch(lines, expect) if st else (False, 0, "no selftest")
    ge_ok, ge_err = r8_ge_batch() if st else (False, "no selftest")
    info = {
        "selftest": st, "batch": ok, "n": n, "ge": ge_ok,
        "src": sha256_file(R8_SRC),
    }
    return info, [
        ("radix8_selftest", st, stout),
        ("radix8_python_c11_full_range", ok and match, f"n={n} {berr}"),
        ("radix8_ge512", ge_ok, ge_err),
    ]
