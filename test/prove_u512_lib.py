...
GO_SRC = ROOT / "test" / "oracles" / "u512_go_oracle.go"
UMAX = (1 << 256) - 1
OP = {"MUL": 1, "MULDIV": 2}


def run(cmd: list[str], inp: str | bytes | None = None, timeout: int = 120,
        cwd: str | None = None) -> subprocess.CompletedProcess:
    text = not isinstance(inp, (bytes, bytearray))
    return subprocess.run(cmd, input=inp, capture_output=True, text=text,
...
def ri_dict(name: str, a: int, b: int, d: int, n: int, q: int, r: int,
            steps: int, status: int) -> dict:
    return {
        "name": name, "a": hex(a), "b": hex(b), "d": hex(d), "n": hex(n),
        "q": hex(q), "r": hex(r), "steps": steps, "status": status,
    }


def ri_vectors(rout_phantom: int) -> list[tuple[str, int, int, int]]:
    return [
        ("small_7x9_div2", 7, 9, 2),
        ("two255_x2_div3", 1 << 255, 2, 3),
        ("getAmountOut_1813", 997000, 20000, 10997000),
        ("phantom_997_rout_1997", 997, rout_phantom, 1997),
        ("n_lt_d_q0", 1, 1, 2),
        ("wrap_borrow_2_x_2_255_div_umax", 2, 1 << 255, UMAX),
        ("nonzero_rem_3x5_div4", 3, 5, 4),
        ("exact_100x200_div50", 100, 200, 50),
    ]
