...
and without the rest of the campaign. Schema 1.10.
Binds packed LCR2 bytes to SafeMath ref_amount_out and packed LCR2-full
bytes to FullMath ref_amount_out_full (EXACT/OVERPAID/PHANTOM/guards).
"""
...
def main() -> int:
    ...
    if ev.get("schema") != "LIN_U512_COPROCESSOR_EVIDENCE_1.10":
        return fail("schema")
    proofs = ev.get("lip1_proofs") or []
    if len(proofs) < 34:
        return fail(f"need >=34 LIP1 proofs, got {len(proofs)}")
    roots = {
    ...
    for p in proofs:
        ...
        if tree == "LCR2_FULL":
            ...
            st = full_by.get(name)
            if not fld or not st:
                return fail(f"LCR2_FULL bind {name}")
            ain, rin, rout, aout, steps, status = fld
            if steps == 0 or steps != int(st["steps"]) or status != int(st["status"]):
                return fail(f"LCR2_FULL steps {name}")
            if ain != parse_int(st["ain"]) or aout != parse_int(st["aout"]):
                return fail(f"LCR2_FULL pack {name}")
            rst_s, _ = ref_amount_out(ain, rin, rout)
            rst_f, ref_f = ref_amount_out_full(ain, rin, rout)
            if name.startswith("FULL_EXACT_INPUT_"):
                if status != 1 or rst_f != 1 or aout != ref_f or aout != parse_int(st["onchain"]):
                    return fail(f"LCR2_FULL EXACT packed {name}")
                n_fex += 1
            elif name.startswith("FULL_OVERPAID_INPUT_"):
                if status != 1 or rst_f != 1 or aout != ref_f or aout == parse_int(st["onchain"]):
                    return fail(f"LCR2_FULL OVERPAID packed {name}")
                n_fov += 1
            elif name == "PHANTOM_APPROVE":
                if status != 1 or rst_s != -2 or rst_f != 1 or aout != ref_f:
                    return fail("LCR2_FULL PHANTOM packed")
                n_fph += 1
            elif name.startswith("FULL_GUARD_ZERO_"):
                if status != -1 or rst_f != -1:
                    return fail("LCR2_FULL GUARD_ZERO packed")
                n_fz += 1
            elif name.startswith("FULL_GUARD_FEE_OVERFLOW_"):
                if status != -2 or rst_f != -2:
                    return fail("LCR2_FULL GUARD_FEE packed")
                n_ffee += 1
            else:
                return fail(f"LCR2_FULL class {name}")
    ...
    if n_fex < 3 or n_fov < 3 or n_fz < 1 or n_ffee < 1 or n_fph < 1:
        return fail(f"LCR2_FULL inclusion exact={n_fex} over={n_fov} z={n_fz} fee={n_ffee} ph={n_fph}")
