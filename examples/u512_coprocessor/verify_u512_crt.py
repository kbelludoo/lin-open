...
# -*- coding: utf-8 -*-
"""
Zero-LIN FullMath CRT + Newton + OpenSSL auditor for schema 1.15 evidence.json.

Python only decodes JSON / hex. CRT reconstructs n==a*b (prod0 wrapping,
mm via Mersenne rotate-and-add, prod1 from CRT). Newton inverse checks
q*d+r==n. OpenSSL dgst recomputes the LRI1 Merkle root (FIPS 180-4 abc
vector). No LinVM, no gcc, no rustc.
...
from u512_crt_fullmath import ge256, identity512, le256, mul256, muldiv256  # noqa: E402
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: verify_u512_crt.py <evidence.json>")
        return 2
    ...
    for i, leaf in enumerate(leaves):
        ...
        if st != 1 or not limbs_eq_int(prod[:N], nlo) or not limbs_eq_int(prod[N:], nhi):
            return fail(f"{leaf.get('name')} n!=a*b CRT")
        ...
        if not identity512(nn[:N], nn[N:], qq, le256(d), rem):
    ...
    for leaf in ev.get("lcr2_leaves") or []:
        ...
        if not bind_muldiv(ain, rin, rout, aout):
            return fail(f"LCR2 {cls} crt bind")
    ...
    print("PASS crt_fullmath remainder identity + openssl merkle")
    print("lri1_crt_identity", len(names))
    print("lri1_match", root)
    print("lcr2_crt_bind", n_ex + n_ov)
