...
# -*- coding: utf-8 -*-
"""
Zero-LIN Karatsuba + Barrett + OpenSSL auditor for schema 1.15 evidence.json.

Python only decodes JSON / hex. Karatsuba reconstructs n==a*b.
Barrett checks q*d+r==n. OpenSSL dgst recomputes the LRI1 Merkle
...
from u512_karatsuba_barrett import ge_limbs, identity512, le256, mul256, muldiv256  # noqa: E402
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: verify_u512_karatsuba.py <evidence.json>")
        return 2
    ...
    for i, leaf in enumerate(leaves):
        ...
        if st != 1 or not limbs_eq_int(prod[:N], nlo) or not limbs_eq_int(prod[N:], nhi):
            return fail(f"{leaf.get('name')} n!=a*b karatsuba")
    ...
    for leaf in ev.get("lcr2_leaves") or []:
        ...
        if not bind_muldiv(ain, rin, rout, aout):
            return fail(f"LCR2 {cls} karatsuba bind")
    ...
    print("PASS karatsuba_barrett remainder identity + openssl merkle")
    print("lri1_karatsuba_identity", len(names))
    print("lri1_match", root)
    print("lcr2_karatsuba_bind", n_ex + n_ov)
