...
# -*- coding: utf-8 -*-
"""
Zero-LIN Knuth Algorithm D + OpenSSL auditor for schema 1.15 evidence.json.

Python only decodes JSON / hex. Knuth D (not restoring bit-serial)
checks n==a*b and q*d+r==n. OpenSSL dgst recomputes the LRI1 Merkle
...
from u512_knuth_d import ge_limbs, identity512, le256, mul256, muldiv256  # noqa: E402
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: verify_u512_knuth_d.py <evidence.json>")
        return 2
    ...
    for i, leaf in enumerate(leaves):
        ...
        if st != 1 or not limbs_eq_int(prod[:N256], nlo) or not limbs_eq_int(prod[N256:], nhi):
        ...
        if not limbs_eq_int(rem[:N256], r):
    ...
    for leaf in ev.get("lcr2_leaves") or []:
        ...
        if not bind_muldiv(ain, rin, rout, aout):
            return fail(f"LCR2 {cls} knuth_d bind")
    ...
    print("PASS knuth_d remainder identity + openssl merkle")
    print("lri1_knuth_d_identity", len(names))
    print("lri1_match", root)
    print("lcr2_knuth_d_bind", n_ex + n_ov)
