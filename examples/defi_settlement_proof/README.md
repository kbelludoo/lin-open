...
normalizada, não restoring bit-serial)); ge em 512 bits com oito folhas LGE1 publicadas (árvore
potência de 2). Consenso Python `int` + C11 16/64 + radix-2^32 + rustc radix-2^64 + Python radix-2^8 + Knuth D + Node BigInt + OpenSSL BN
+ Go + Java + GMP + Perl + GNU bc no lote n=1060. Merkle LNR1 (Python / Node / C11
`lin_sha256` / OpenSSL) rejeita tamper de 1 bit. Schema 1.12: verificadores zero-LIN em
