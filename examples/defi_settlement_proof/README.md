...
+ GNU MP `mpz` via `dlopen libgmp.so.10`); ge em 512 bits com oito
folhas LGE1 publicadas (árvore potência de 2). Consenso Python `int` + C11 + Node BigInt
+ OpenSSL BN + Go + Java + GMP no lote n=1060. Merkle LNR1 (Python / Node / C11 `lin_sha256` / OpenSSL)
rejeita tamper de 1 bit. Schema 1.7: verificadores zero-LIN em **Python e Node** recomputam
...
ligando os 208 bytes empacotados a `ref_amount_out` sem LinVM. C11 `lin_sha256` verifica
o recibo LIP1 de um EXACT e o phantom 512-bit. GMP recomputa `mulDiv(fee,rout,den)` nas
6 liquidações EXACT/OVERPAID.
