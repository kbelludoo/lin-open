...
de 32 limbs e `q*d+r==n`; ge em 512 bits; consenso Python `int` + C11 16-bit + C11
64-bit `__int128`; Merkle LNR1 (Python / Node / C11 `lin_sha256` / OpenSSL) rejeita
tamper de 1 bit. O verificador zero-LIN **recomputa** a raiz da campanha a partir das
folhas publicadas no JSON (não há merkle opaco). Fatia LCR2: 3 `EXACT_INPUT` + 3
