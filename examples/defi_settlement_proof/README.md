...
schoolbook + rustc radix-2^64 + Node.js BigInt + OpenSSL BIGNUM via `dlopen libcrypto.so.3` + Go `math/big`
...
potência de 2). Consenso Python `int` + C11 16/64 + radix-2^32 + rustc radix-2^64 + Node BigInt + OpenSSL BN
+ Go + Java + GMP + Perl + GNU bc no lote n=1060. Merkle LNR1 (Python / Node / C11
`lin_sha256` / OpenSSL) rejeita tamper de 1 bit. Schema 1.10: verificadores zero-LIN em
...
GMP, Perl, radix-2^32, rustc radix-2^64 e GNU bc recomputam `mulDiv(fee,rout,den)` nas 6 liquidações
EXACT/OVERPAID; Perl, radix-2^32, rustc radix-2^64 e GNU bc também fecham o phantom FullMath-width.
...
da árvore LRI1 e o `mulDiv` das fatias LCR2/phantom — três larguras de limb (16/32/64)
não podem esconder o mesmo bug de carry.
