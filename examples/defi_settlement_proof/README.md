...
potência de 2). Consenso Python `int` + C11 16/64 + radix-2^32 + rustc radix-2^64 + Python radix-2^8 + Knuth D + CRT/Newton + Karatsuba/Barrett + Toom-3/Goldschmidt + Node BigInt + OpenSSL BN
+ Go + Java + GMP + Perl + GNU bc no lote n=1060. Merkle LNR1 (Python / Node / C11
`lin_sha256` / OpenSSL) rejeita tamper de 1 bit. Schema 1.15: verificadores zero-LIN em
...
GMP, Perl, radix-2^32, rustc radix-2^64, Python radix-2^8, Knuth D, CRT, Karatsuba/Barrett, Toom-3/Goldschmidt e GNU bc recomputam `mulDiv(fee,rout,den)` nas 6 liquidações
EXACT/OVERPAID; Perl, radix-2^32, rustc radix-2^64, Python radix-2^8, Knuth D, CRT, Karatsuba/Barrett, Toom-3/Goldschmidt e GNU bc também fecham o phantom FullMath-width.
