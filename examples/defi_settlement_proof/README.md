...
+ OpenSSL BIGNUM via `dlopen libcrypto.so.3` + Go `math/big` + Java `BigInteger`
+ GNU MP `mpz` via `dlopen libgmp.so.10` + Perl `Math::BigInt::Calc`, não GMP); ge em 512 bits com oito
folhas LGE1 publicadas (árvore potência de 2). Consenso Python `int` + C11 + Node BigInt
+ OpenSSL BN + Go + Java + GMP + Perl no lote n=1060. Merkle LNR1 (Python / Node / C11 `lin_sha256` / OpenSSL)
rejeita tamper de 1 bit. Schema 1.8: verificadores zero-LIN em **Python, Node e Perl** recomputam
as raízes LNR1, LCR2 (largura SafeMath), LCR2-full (512-bit numerator via `settle_u512_swap`),
LGE1 (cmp/ge 512) **e** LRI1 (identidade de resto: `n==a*b` e `q*d+r==n` sobre o produto
de 512 bits publicado) a partir das folhas publicadas, **e** verificam recibos LIP1
de **todas as 8 folhas LCR2** (3 EXACT + 3 OVERPAID + `GUARD_ZERO` + `GUARD_OVERFLOW`)
ligando os 208 bytes empacotados a `ref_amount_out` **e de todas as 9 folhas LCR2-full**
ligando os 208 bytes a `ref_amount_out_full` sem LinVM. C11 `lin_sha256` verifica
o recibo LIP1 de um EXACT, o phantom 512-bit LRI1 **e** o `PHANTOM_APPROVE` LCR2-full.
GMP e Perl recomputam `mulDiv(fee,rout,den)` nas 6 liquidações EXACT/OVERPAID; Perl
também fecha o phantom FullMath-width.
