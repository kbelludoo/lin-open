...
+ OpenSSL BIGNUM via `dlopen libcrypto.so.3` + Go `math/big`); ge em 512 bits com oito
folhas LGE1 publicadas (árvore potência de 2). Consenso Python `int` + C11 + Node BigInt
+ OpenSSL BN + Go no lote n=1060. Merkle LNR1 (Python / Node / C11 `lin_sha256` / OpenSSL)
rejeita tamper de 1 bit. Schema 1.5: verificadores zero-LIN em **Python e Node** recomputam
...
de 512 bits publicado) a partir das folhas publicadas.
