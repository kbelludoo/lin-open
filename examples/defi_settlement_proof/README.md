...
+ OpenSSL BIGNUM via `dlopen libcrypto.so.3` + Go `math/big` + Java `BigInteger`); ge em 512 bits com oito
folhas LGE1 publicadas (árvore potência de 2). Consenso Python `int` + C11 + Node BigInt
+ OpenSSL BN + Go + Java no lote n=1060. Merkle LNR1 (Python / Node / C11 `lin_sha256` / OpenSSL)
rejeita tamper de 1 bit. Schema 1.6: verificadores zero-LIN em **Python e Node** recomputam
...
(folha + irmãos Merkle + raiz) sem o resto da campanha — o phantom 512-bit e o
`GUARD_OVERFLOW` u256 (`-2`) são auditáveis em isolamento.
