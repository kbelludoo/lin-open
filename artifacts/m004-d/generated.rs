...
pub mod AstModule {
    ...
    pub fn createNode(kind: NodeKind, id: String, line: i64) -> SyntaxNode {
        return SyntaxNode { kind: kind, id: id, line: line };
    }
}
pub mod ParserModule {
    use super::*;
    pub use super::AstModule::{NodeKind, SyntaxNode, createNode};
    pub fn parseFunctionSignature(sig: String, line: i64) -> SyntaxNode {
        return createNode(NodeKind::Function, sig, line);
    }
    pub fn parseModuleHeader(name: String, line: i64) -> SyntaxNode {
        return createNode(NodeKind::Module, name, line);
    }
}
pub mod SemanticModule {
    use super::*;
    pub use super::AstModule::{NodeKind, SyntaxNode};
    pub fn validateNode(node: SyntaxNode) -> bool {
        match node {
            SyntaxNode { kind: NodeKind::Function, id, line } if line > 0 => {
                return true;
            }
            SyntaxNode { kind: NodeKind::Module, id, line } if line > 0 => {
                return true;
            }
            _ => {
                return false;
...
pub mod EmitterModule {
    use super::*;
    pub use super::AstModule::{NodeKind, SyntaxNode};
    pub fn emitRustDeclaration(node: SyntaxNode) -> String {
        match node {
            SyntaxNode { kind: NodeKind::Function, id, line: _ } => {
                return _lia_val(_lia_cat(&_lia_cat(&"pub fn ",&id),&"() {}"));
            }
            SyntaxNode { kind: NodeKind::Module, id, line: _ } => {
                return _lia_val(_lia_cat(&_lia_cat(&"pub mod ",&id),&" {}"));
            }
            _ => {
                return _lia_val("// unknown node");
            }
        }
    }
}
pub use AstModule::{NodeKind, SyntaxNode};
pub use ParserModule::{parseFunctionSignature, parseModuleHeader};
pub use SemanticModule::validateNode;
pub use EmitterModule::emitRustDeclaration;
pub fn compile_pipeline(fnName: String, line: i64) -> String {
    let mut node: SyntaxNode = Default::default();
    let mut valid: bool = false;
    node = parseFunctionSignature(fnName, line);
    valid = validateNode(node.clone());
    if valid == true {
        return _lia_val(emitRustDeclaration(node));
    }
    return _lia_val("ERROR_INVALID_NODE");
}
pub fn compile_module_pipeline(modName: String, line: i64) -> String {
    let mut node: SyntaxNode = Default::default();
    let mut valid: bool = false;
