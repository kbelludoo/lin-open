...
pub fn classify_ast_node(node: AstNode) -> String {
    match node {
        AstNode { kind: AstKind::Ident, name, value: _ } => {
            return _lia_val(_lia_cat(&"IDENT:",&name));
        }
        AstNode { kind: AstKind::Literal, name: _, value } => {
            return _lia_val(_lia_cat(&"LITERAL:",&value));
        }
        AstNode { kind: AstKind::BinaryOp, name, value } if value > 0 => {
            return _lia_val(_lia_cat(&"BINOP_POS:",&name));
        }
        AstNode { kind: AstKind::BinaryOp, name, value: _ } => {
            return _lia_val(_lia_cat(&"BINOP:",&name));
        }
        AstNode { kind: AstKind::MatchBlock, name: _, value: _ } => {
            return _lia_val("MATCH");
        }
        _ => {
            return _lia_val("UNKNOWN");
        }
    }
}
pub fn unify_types_with_match(a: String, b: String) -> String {
...
pub fn infer_param_type_with_match(name: String) -> String {
    match name.as_str() {
        "i" | "j" | "k" | "n" | "len" | "idx" | "count" => {
            return _lia_val("int");
        }
        "str" | "s" | "ch" | "pad" | "name" | "key" | "value" | "id" => {
            return _lia_val("str");
        }
        "flag" | "ok" | "done" | "valid" => {
            return _lia_val("bool");
        }
        "items" | "list" | "arr" | "args" => {
            return _lia_val("list");
        }
        "opts" | "config" | "options" | "obj" => {
            return _lia_val("map");
        }
        "fn" | "cb" | "callback" | "handler" => {
            return _lia_val("fn");
        }
        _ => {
            return _lia_val("any");
