/**
 * Host loader: LIN source is src/emit_mojo.lin
...
const LIN = path.join(path.dirname(fileURLToPath(import.meta.url)), 'emit_mojo.lin');
let mod = null;
...
export function getMod() {
  ...
  const tmp = path.join(os.tmpdir(), `lin_emit_mojo_${process.pid}.cjs`);
...
export function emitMojo(...args) {
  return getMod().emitMojo(...args);
}
