/**
 * Host loader: LIN source is src/compare_targets.lin
...
const LIN = path.join(path.dirname(fileURLToPath(import.meta.url)), 'compare_targets.lin');
let mod = null;
...
export function getMod() {
  ...
  const tmp = path.join(os.tmpdir(), `lin_compare_targets_${process.pid}.cjs`);
...
export function compareTargets(...args) {
  return getMod().compareTargets(...args);
}
