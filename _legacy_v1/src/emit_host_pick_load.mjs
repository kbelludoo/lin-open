/**
 * Host loader: LIN source is src/emit_host_pick.lin
...
const LIN = path.join(path.dirname(fileURLToPath(import.meta.url)), 'emit_host_pick.lin');
let mod = null;
...
export function getMod() {
  ...
  const tmp = path.join(os.tmpdir(), `lin_emit_host_pick_${process.pid}.cjs`);
...
export function emitHostPick(...args) {
  return getMod().emitHostPick(...args);
}
