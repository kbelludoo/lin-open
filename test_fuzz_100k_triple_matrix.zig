...
pub fn main() !void {
    ...
    try stdout.print("\n================================================================================\n", .{});
    try stdout.print("=== LIN-MIR-FUZZ-003B: 100,000 TRIPLE EXECUTIONS (10,000 CFG PROGS x 10 INPUTS) ===\n", .{});
    try stdout.print("================================================================================\n\n", .{});

    var prng = std.Random.DefaultPrng.init(424242);
    const rand = prng.random();

    const total_programs: usize = 10_000;
    const inputs_per_program: usize = 10;
    const total_target_executions: usize = total_programs * inputs_per_program; // 100,000
    ...
    for (0..total_programs) |prog_idx| {
        ...
        // 1. Generic JIT Compilation directly from MirFunction
        var jit_compiled = try generic_jit.GenericJitEngine.compile(func);
        defer jit_compiled.deinit();

        // 2. Run 10 diverse inputs across VM, AOT, and Generic JIT
        for (0..inputs_per_program) |_| {
            ...
            if (vm_res == aot_res and aot_res == jit_res) {
                ...
                std.debug.print("[FAIL] Mismatch in prog {d}: VM={d}, AOT={d}, JIT={d}\n", .{ prog_idx, vm_res, aot_res, jit_res });
                return error.TripleMatrixMismatch;
            }
        }
    }

    const elapsed_ns = timer.read();
    const rate = (@as(f64, @floatFromInt(total_target_executions)) / @as(f64, @floatFromInt(elapsed_ns))) * 1000.0;

    try stdout.print("[FUZZING SUMMARY] 100,000 Triple Executions Verified Across All 3 Backends:\n", .{});
    try stdout.print("                  Total MIR Programs:    {d}\n", .{total_programs});
    try stdout.print("                  Inputs Per Program:    {d}\n", .{inputs_per_program});
    try stdout.print("                  Total Executions:      {d}\n", .{total_target_executions});
    try stdout.print("                  Triple Matches:        {d} (100.0% Exact Parity VM == AOT == JIT)\n", .{total_triple_matches});
    try stdout.print("                  Total Elapsed Time:    {d:.2} ms ({d:.2} M executions/s)\n", .{ @as(f64, @floatFromInt(elapsed_ns)) / 1_000_000.0, rate });
    try stdout.print("                  Triple Divergences:    0\n\n", .{});
    try stdout.print("================================================================================\n", .{});
    try stdout.print("=== GATE PASS: LIN-MIR-FUZZ-003B (100,000 GENERIC TRIPLE EXECUTIONS) ===\n", .{});
    try stdout.print("================================================================================\n\n", .{});
}
